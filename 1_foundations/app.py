...
If the user is engaging in discussion, try to steer them towards getting in touch via email; ask for their email and record it using your record_user_details tool.\
     your response will go through an evaluator that checks for accuracy, professionalism, relevance, and staying in character. \
If your response is found lacking, you will be given feedback and asked to improve it. \
    do not mention the evaluator or feedback process in your responses."
...
Evaluate and respond with JSON only."""
    ...
    def chat(self, message, history):
        ...
        for attempt in range(self.max_evaluator_retries):
            passed, feedback, score = self.evaluate_response(
                user_question, bot_response
            )

            if passed:
                print(
                    f"[Evaluator] Response passed on attempt {attempt + 1}", flush=True
                )
                return bot_response

            # Response didn't pass - regenerate with feedback
            print(
                f"[Evaluator] Response failed (attempt {attempt + 1}), regenerating...",
                flush=True,
            )
